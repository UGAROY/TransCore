package com.transcendss.transcore.util
{
		 ...
		 * src: http://chris.firlingcanada.com/flexdevtips/flex3/dashedlines/srcview/
		 * @author Chris Callendar.
		 */
		public final class GraphicsUtils extends GraphicsUtil
		{
...
//			 * @param dashLength the length of the dash (only applies if dashed is true)
//			 */
//			public static function drawDottedLine(g:Graphics, start:Point, end:Point, 
//											dashed:Boolean = false, dashLength:Number = 1):void {
//				g.moveTo(start.x, start.y);
//				if (dashed) {
//					// the distance between the two points
//					var total:Number = Point.distance(start, end);
...
//			}
			 ...
			 */
			public static function drawRect(g:Graphics, x:Number, y:Number, w:Number, h:Number, cornerRadii:Array = null, dashed:Boolean = true, dashLength:Number=10):void {
				var noCorner:Boolean = (cornerRadii == null) || (cornerRadii.length == 0);
				var drawn:Boolean = false;
				if (dashed) {
					drawLine(g,new Point(x,y), new Point(x+w,y),true,dashLength);
					drawLine(g,new Point(x+w,y), new Point(x+w,y+h),true,dashLength);
					drawLine(g,new Point(x+w,y+h), new Point(x,y+h),true,dashLength);
					drawLine(g,new Point(x,y+h), new Point(x,y),true,dashLength);
					drawn = true;
				}
				else
				{
