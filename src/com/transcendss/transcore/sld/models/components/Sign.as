package com.transcendss.transcore.sld.models.components
{
	...
	{
		public static var TYPE:String = "SIGN";
		...
		public function draw(ac:ArrayCollection, diagramScale:Number, diagramHeight:Number,rteBeginMi:Number):void
		{
			...
			for(var i:int =0; i<signColl.length; i++)
			{
				if(signColl.getItemAt(i) == null)
					continue;
				signSprite = new AssetSymbol(signColl.getItemAt(i) as BaseAsset);
				signID = new Number(String(signColl.getItemAt(i).invProperties.ID.value));
				milePoint = new Number(String(signColl.getItemAt(i).invProperties.REFPT.value));
				featureName = String(signColl.getItemAt(i).invProperties.FEATURE_NAME.value);
				mutcd = String(signColl.getItemAt(i).invProperties.MUTCD.value);
				...
				signSprite.x =  Converter.scaleMileToPixel(milePoint-rteBeginMi,diagramScale);
				signSprite.y = 40;	
				addChild(signSprite);
				/*loader = new Loader();
				...
				loader.y = 40;*/
				signColl.getItemAt(i).symbol = signSprite;
			}  
		}
